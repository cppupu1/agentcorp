...
import { listPmSessions, getPmMessages, deletePmSession, runPmChat, getPmAssistantModelId } from '../services/pm-assistant.js';
import type { AgentStreamCallbacks } from '@agentcorp/agent-core';
import { db, models } from '@agentcorp/db';
import { eq } from 'drizzle-orm';

export function registerPmAssistantRoutes(app: FastifyInstance) {
  app.get('/api/pm-assistant/status', async () => {
  ...
  app.get('/api/pm-assistant/sessions', async () => {
    return listPmSessions();
  });

  app.get<{ Params: { sessionId: string } }>(
    '/api/pm-assistant/:sessionId/messages',
    async (req) => {
      return getPmMessages(req.params.sessionId);
    },
  );

  app.delete<{ Params: { sessionId: string } }>(
    '/api/pm-assistant/:sessionId',
    async (req) => {
      return deletePmSession(req.params.sessionId);
    },
  );

  app.post<{ Body: { sessionId: string; message: string } }>(
    '/api/pm-assistant/chat',
    async (req, reply) => {
      ...
      try {
        await runPmChat({ sessionId, message }, callbacks);
      } catch (err) {
