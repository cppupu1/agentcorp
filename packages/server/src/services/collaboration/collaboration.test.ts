...
vi.mock('./utils.js', () => ({ runAgentOnce: (...args: any[]) => mockRunAgentOnce(...args) }));
...
describe('DebateStrategy', () => {
  it('runs 3 phases: analysis → cross-review → PM synthesis', async () => {
    ...
    mockRunAgentOnce.mockResolvedValue('mock response');
    const ctx = makeCtx();

    await new DebateStrategy().execute(ctx);

    // 2 members analyze + 2 members review + 1 PM synthesize = 5 calls
    expect(mockRunAgentOnce).toHaveBeenCalledTimes(5);
  ...
  it('aborts after analysis if signal is aborted', async () => {
    ...
    mockRunAgentOnce.mockImplementation(async () => {
      ac.abort();
      return 'done';
    });
...
describe('VoteStrategy', () => {
  it('runs voting then PM tallies', async () => {
    ...
    mockRunAgentOnce.mockResolvedValue('方案内容\n【方案标签: 方案A】');
    const ctx = makeCtx();

    await new VoteStrategy().execute(ctx);

    // 2 members vote + 1 PM tally = 3 calls
    expect(mockRunAgentOnce).toHaveBeenCalledTimes(3);
  ...
  it('extracts vote label from response', async () => {
    ...
    const pmCall = mockRunAgentOnce.mock.calls[2];
    expect(pmCall[0]).toBe('pm-1');
    expect(pmCall[1]).toContain('重构方案');
    expect(pmCall[1]).toContain('渐进方案');
  });
...
describe('MasterSlaveStrategy', () => {
  it('runs 3 phases: plan → execute → aggregate', async () => {
    ...
    expect(mockRunAgentOnce).toHaveBeenCalledTimes(4);
    expect(mockRunAgentOnce.mock.calls[0][0]).toBe('pm-1'); // plan
    expect(mockRunAgentOnce.mock.calls[3][0]).toBe('pm-1'); // aggregate
  ...
  it('handles master planning failure', async () => {
    const { MasterSlaveStrategy } = await import('./master-slave.js');
    mockRunAgentOnce.mockRejectedValueOnce(new Error('plan failed'));
    const ctx = makeCtx();
